...
CoreAlg <- function(X, y, absolute, abs_method){
  ...
  tryCatch({
    ...
    write(paste("1: ",rmses),stderr())
    write(paste("2: ",mn),stderr())
    write(paste("out[[mn]]: ",error),stderr())
  })
...
doPerm <- function(perm, X, Y, absolute, abs_method){
  ...
  while(itor <= perm){
    ...
    tryCatch({
      ...
      result <- 0
      write(paste("CoreAlg: ",error),stderr())
    })
...
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score'){
  ...
  output <- bind_cols(
    mclapply(1:mixtures,function(itor){
      y <- Y[,itor]
  ...
  output2 <- output[-1,]
  colnames(output2) <- output[1,]
  obj <-as.data.frame(sapply(output2, as.numeric)) #matrix(as.numeric(unlist(output2)),nrow=nrow(output2))
  rownames(obj) <- header[-1]
  return(obj)
}
...
#需同一個file內
# sig_matrix <- "LM22.txt"
# mixture_file <- "20220321_GSE131928.txt"
# Results <-  CIBERSORT(sig_matrix , mixture_file , perm=100, QN=TRUE, absolute=FALSE, abs_method='sig.score')
