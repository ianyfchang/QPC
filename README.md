...
CIBERSORTx is an analytical tool  to imput gene expression profiles and provide an estimation of the abundances of member cell types in a mixed cell population, using gene expression data.
CIBERSORTx is only freely available for academic users and could not be directly included in this package. To use CIBERSORT with this package, you need to register on the [cibersort website](https://cibersortx.stanford.edu/), obtain a license, and download the CIBERSORT source code.                                                   
